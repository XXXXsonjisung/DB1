 ...
 * 이 때, 죄회된 행들의 집합을 "RESULT SET"(조회 결과의 집합) 이라고 한다.
 * 
 * -RESULT SET은 0개 이상의 행을 포함하 수 있다.
 * 왜 0개? 조건의 맞는 행이 없을 수도 있어서.
 * 
 * 
 * 
 */
...
--< 컬럼 값 산술 연산>
-- 컬럼 값 : 테이블 내 한 칸(== 한셀 ) 에 작성된 값 (DATA)
 ...
 * 
 * 컬럼녕 AS "별칭" : 별칭 띄어쓰기 O, 특수문자 O, 문자 O
 *
 * AS 생략 가능
 *
 */

SELECT SYSDATE -1 "하루 전", SYSDATE  AS 현재시간, SYSDATE +1내일
FROM DUAL;
...
OR DEPT_CODE IS NULL; -- 부서코드 없는 2명 포함14명
