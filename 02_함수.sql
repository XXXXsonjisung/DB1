...
-- 단일 행 함수 : N개의 값을 읽어서 N개의 결과를 반화

-- 그룹 함수 : N개의 값을 읽어서 1개의 결과를 반환 (합계, 평균, 최대, 최소)

-- 함수는 SELECT 문의
-- SELECT절, WHERE절, GROUP BY절, HAVING절 사용 가능
...
SELECT INSTR('AABAACAABBAA','B') FROM DUAL; -- 3 첫번째 B

-- 문자열을 5번째 문자 부터 검색하여 첫번째 B 위치 조회

SELECT INSTR('AABAACAABBAA','B', 5 ) FROM DUAL; -- 9

-- 문자열을 5번째 문자 부터 검색해서 두번째 B위치 조회
...
-- EMPLOYEE 테이블에서 사번이 짝수인 사워의 사번, 이름 조회
SELECT EMP_ID,EMP_NAME 
FROM EMPLOYEE
WHERE MOD (EMP_ID,2)=0;


-- EMPLOYEE 테이블에서 사번이 홀수인 사워의 사번, 이름 조회
SELECT EMP_ID,EMP_NAME 
FROM EMPLOYEE
WHERE MOD (EMP_ID,2)<> 0;
